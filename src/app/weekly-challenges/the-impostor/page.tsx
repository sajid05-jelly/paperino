...
export default function TheImpostorPage() {
  ...
  if (gameState === "intro") return (
    <div className="min-h-screen p-6 flex flex-col items-center justify-center max-w-xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full glass-panel vision-glass p-8 rounded-3xl border border-yellow-500/20 text-center flex flex-col items-center gap-6">
        <div className="w-16 h-16 rounded-2xl bg-yellow-500/20 flex items-center justify-center text-yellow-400 border border-yellow-500/30 shadow-[0_0_30px_rgba(250,204,21,0.3)]">
          <Search size={32} />
        </div>
        <div>
          <h1 className="text-3xl font-extrabold text-white mb-2">The Impostor</h1>
          <p className="text-sm text-gray-300 max-w-sm mx-auto leading-relaxed">Read the clues carefully and identify the correct answer. Every clue matters — one answer is logically certain!</p>
        </div>
        <div className="w-full bg-black/40 p-4 rounded-2xl border border-white/5 space-y-2 text-xs text-left text-gray-300">
          <p>• Read all <strong>4–5 clues</strong> carefully.</p>
          <p>• Use logic to determine the <strong>one correct answer</strong>.</p>
          <p>• Every scenario has exactly <strong>one valid answer</strong>.</p>
          <p>• Speed gives a bonus — but accuracy is required.</p>
        </div>
  ...
  if (gameState === "loading") return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md w-full glass-panel vision-glass p-8 rounded-3xl border border-yellow-500/20 text-center flex flex-col items-center gap-6">
        ...
        <div className="w-full bg-white/5 rounded-full h-2 overflow-hidden"><motion.div className="bg-gradient-to-r from-yellow-500 to-amber-400 h-full rounded-full" animate={{ x: ["-100%", "200%"] }} transition={{ repeat: Infinity, duration: 1.8 }} /></div>
      </motion.div>
    </div>
  );
  ...
  if (gameState === "result" && resultData) return (
    <div className="min-h-screen p-6 flex items-center justify-center">
      <GameResult score={resultData.score} durationMs={resultData.durationMs} rank={resultData.rank} isOfficial={resultData.isOfficial} gameId="the-impostor" gameName="The Impostor" onViewLeaderboard={() => {}} onPlayAgain={() => window.location.reload()} onBackToHub={() => router.push("/weekly-challenges")} leaderboard={resultData.leaderboard} wasAlreadyCompleted={wasAlreadyCompleted} />
    </div>
  );
  ...
  const rulesContent = (
    <ul className="space-y-2.5 list-disc list-inside text-sm">
      ...
      <li>Use logical deduction to find the <strong>one correct answer</strong>.</li>
      <li>Speed gives a bonus score, but you must be accurate.</li>
    </ul>
  );
        ...
        <div className="grid grid-cols-2 gap-3">
          {puzzle?.options.map((opt, i) => (
            <button
              ...
              className={`p-5 rounded-2xl border text-sm font-semibold transition-all flex items-center gap-3 ${selectedAnswer === opt ? "bg-yellow-500/25 border-yellow-400/60 text-yellow-200 shadow-[0_0_20px_rgba(250,204,21,0.2)]" : "bg-white/5 border-white/10 text-white hover:bg-yellow-500/10 hover:border-yellow-500/30"}`}
            >
              <User size={18} className={selectedAnswer === opt ? "text-yellow-400" : "text-gray-500"} />
              {opt}
            </button>
          ))}
        </div>
