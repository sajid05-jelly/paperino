...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!files || files.length === 0 || !subject) {
      return NextResponse.json({ error: "Please provide at least one file and a subject name." }, { status: 400 });
    }

    if (files.length > 5) {
      return NextResponse.json({ error: "Maximum of 5 PDF files allowed per analysis." }, { status: 400 });
    }
    ...
    if (!ocrMode && combinedText.trim().length < 50) {
      return NextResponse.json({ errorType: "NEEDS_OCR", error: "Image-based PDF detected. Switch to OCR mode." }, { status: 400 });
    }
...
Your goal is to deeply analyze the provided Previous Year Question (PYQ) papers for the subject: "${subject}".
...

{
  "repeatedQuestions": [
    {
      ...
      "frequencyCount": number, // Number of times it appeared
      "insight": "string" // e.g., "Always asked in Part B"
    }
  ],
  "importantTopics": [
    {
  ...
  "unitImportance": [
    {
      "unit": "string", // e.g., "Unit 1", "Module 3", or "Topic Name" if units aren't explicit
      "probabilityScore": number // 0-100
    }
  ],
  "highProbabilityQuestions": [
    "string" // 3 to 5 highly predicted conceptual questions
  ],
  "summaryInsight": "string" // A concluding piece of smart advice, e.g. "Focus heavily on Unit 3 and 4 as they constitute 60% of repeated questions."
}
...
`;
    ...
    let parsedData;
    try {
      // Clean markdown wrappers if the AI included them despite instructions
      let cleanedText = responseText.replace(/^```json\s*/i, "").replace(/^```\s*/i, "").replace(/\s*```$/i, "").trim();
      parsedData = JSON.parse(cleanedText);
    } catch (parseError: any) {
      console.error("JSON Parsing Error:", parseError.message);
      console.error("Raw AI Response:", responseText);
      return NextResponse.json({ error: "The AI generated an improperly formatted response. Please click predict again." }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: error.message || "Failed to analyze PYQs via Gemini AI." }, { status: 500 });
  }
}
