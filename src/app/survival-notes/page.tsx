...
export default function SurvivalNotesPage() {
  ...
  const handleAdminAction = async (noteId: string, action: "approve" | "reject" | "delete") => {
    ...
    try {
      if (action === "approve") {
        const noteSnap = await getDocs(query(collection(db, "survival_notes"), where("status", "==", "pending")));
  ...
  return (
    <div className="w-full min-h-screen bg-[var(--background)] text-white py-8 relative overflow-hidden selection:bg-violet-500/30">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12 md:py-16">
        ...
        {/* Dropdown selectors */}
        <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.06] rounded-3xl p-6 shadow-lg max-w-4xl mx-auto mb-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Select Department</label>
              <select
                value={selectedDept}
                onChange={(e) => { setSelectedDept(e.target.value); setSelectedSem(""); setSelectedSubject(""); }}
                className="w-full bg-black/60 border border-white/[0.08] focus:border-violet-500/30 rounded-xl p-3.5 text-white outline-none cursor-pointer"
              >
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Select Semester</label>
              <select
                disabled={!selectedDept}
                value={selectedSem}
                onChange={(e) => { setSelectedSem(e.target.value); setSelectedSubject(""); }}
                className="w-full bg-black/60 border border-white/[0.08] focus:border-violet-500/30 rounded-xl p-3.5 text-white outline-none cursor-pointer disabled:opacity-40"
              >
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Select Subject</label>
              <select
        ...
        {selectedSubject ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              {showAddForm && (
                <form onSubmit={handleSubmitNote} className="backdrop-blur-xl bg-white/[0.03] border border-violet-500/25 rounded-3xl p-6 shadow-md space-y-5 animate-in fade-in slide-in-from-top-4 duration-300">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Write your tip (will be anonymous by default)</label>
                    <textarea
                      rows={4}
                      value={newContent}
                      onChange={(e) => setNewContent(e.target.value)}
                      placeholder="Share exam tips, unit importance, viva questions, or assignment details..."
                      className="w-full bg-black/60 border border-white/[0.08] focus:border-violet-500/30 rounded-xl p-3.5 text-white outline-none"
                    />
                  </div>
              ...
              {loading ? (
                ...
                <div className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.06] rounded-3xl p-12 text-center text-gray-500 space-y-4">
                  <Bot size={44} className="mx-auto text-violet-500/40" />
                  <p className="text-sm">No approved notes found for this subject yet.</p>
                  {user && (
                    <button 
                ...
                <div className="space-y-5">
                  {notes.map(note => {
                    ...
                    return (
                      <div key={note.id} className="backdrop-blur-xl bg-white/[0.02] border border-white/[0.06] hover:border-violet-500/15 rounded-3xl p-6 transition-all duration-300 relative group/card">
                        
                        {/* Category Badge & Contributor */}
                        <div className="flex justify-between items-center mb-4">
                          <span className="px-3 py-1 bg-violet-500/10 border border-violet-500/20 text-violet-300 rounded-full text-[10px] font-bold tracking-wide">
                            {note.category}
                          </span>
                          <span className="text-[10px] text-gray-500 font-bold flex items-center gap-1">
                            <UserCheck size={12} /> Senior Contributor
                          </span>
                        </div>
                ...
                {pendingNotes.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {pendingNotes.map(pNote => (
                      <div key={pNote.id} className="p-4 bg-amber-500/[0.02] border border-amber-500/25 rounded-2xl space-y-3">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-[9px] text-gray-500 font-medium">By: {pNote.contributorName}</span>
                        </div>
                        <p className="text-xs text-gray-300 leading-relaxed max-h-24 overflow-y-auto whitespace-pre-line font-medium border-b border-white/5 pb-2">
                          "{pNote.content}"
                        </p>
