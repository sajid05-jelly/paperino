        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 w-full relative z-10">
          {semesters.map((sem, i) => (
            <Link key={sem} href={`/btech/semesters/${sem}`}>
              <div className="vision-glass p-5 md:p-6 rounded-[1.5rem] group cursor-pointer vision-hover h-full flex flex-col justify-between min-h-[140px] animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${i * 100}ms`, animationFillMode: 'both' }}>
